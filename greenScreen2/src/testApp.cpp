...
*/
	...
	if (go || editing) {
		cout << "                                go" << endl;
		greenFBO.allocate(newPic.width, newPic.height);
		comp.allocate(newPic.width, newPic.height, 4);
		if (newPic.isAllocated()) {

			greenscreen.setPixels(newPic.getPixelsRef());
...
void testApp::draw() {
	...
	greenFBO.begin(); 
		...
		ofSetColor (255); 
		bgImg.draw(0,0, newPic.width, newPic.height); 
...
#endif
	if (wingState == 2) {
	greenscreen.setBgColor(newPicDupe.getPixelsRef().getColor(x, y));
	cout << "get color" << endl; 
	} 
	/*
	else if (wingState == 2) {
...
void testApp::imageDownloaded(string &path) {
	//newPic.loadImage(camera.getLastImagePath());
	newPic.clear(); 
	//newPic.loadImage("IMG_0015.JPG");
	//newPic.loadImage(path);
	
	newPic.loadImage(camera.lastImagePath);
	newPicDupe.loadImage(camera.lastImagePath);
	cout << "last image path: " << camera.lastImagePath << endl; 
	//newPic.allocate (512, 360, OF_IMAGE_COLOR); 
