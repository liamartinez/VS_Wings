...
//        }
    ...
    bool CanonCameraWrapper::downloadImage(EdsDirectoryItemRef directoryItem){
		...
		if (success)
			...
			evtImageDownloaded.notify(this, lastImagePath);
		cout << "path is: " << lastImagePath << endl; 
				
			//cout << "something happens here that I deleted" << endl; 
        return success;
    }
	...
	EdsError EDSCALLBACK CanonCameraWrapper::handlePropertyEvent(EdsPropertyEvent inEvent,  EdsPropertyID inPropertyID, EdsUInt32 inParam, EdsVoid * inContext){
		...
		switch(inPropertyID){
			...
			break;
			case  kEdsPropID_BodyIDEx               : prop = "kEdsPropID_BodyID";
			break;
